...
object MoveDirectry {
  def main(args: Array[String]): Unit = {
    val dirList = newProcess("cd ../before/src && ls").lines.toList
    dirList.foreach { projectName =>
      val gitText: String = newProcess(s"git -C ../before/src/$projectName remote -v").lines.head.dropRight(8)
  ...
  def newProcess(cmd: String): ProcessBuilder = Process(Seq("sh", "-c", cmd))
  ...
  def mkdir(dirNameArray: Array[String], domainName: String, gitText: String): Unit = {
    ...
    newProcess(s"mkdir -p ../before/src/$domainName/$dirName").run
    println(s"Sucess!\n$domainName/$dirName")
  }
  ...
  def mv(projectName: String, domainName: String, dirNameArray: Array[String]) = {
    ...
    newProcess(s"mv ../before/src/$projectName ../before/src/$gitProjectDirName").run
    //「p/shokohara-123456/r/project5」のようにfullPathString
    val fullDirName: String = dirNameArray.toSeq.mkString("/")
    newProcess(s"mv ../before/src/$gitProjectDirName/ ../before/src/$domainName/$fullDirName").run
  }
}
